

  class ONI_Recon_Rifleman_pack: ONI_Recon_Rucksack
  {
    scope=1;
    class TransportMagazines
    {
     class _xx_OPTRE_32Rnd_762x51_Mag {count=5;magazine="OPTRE_32Rnd_762x51_Mag";};
     class _xx_OPTRE_8Rnd_127x40_Mag {count=2;magazine="OPTRE_8Rnd_127x40_Mag";};
    };
    class TransportItems
    {
     class _xx_ACE_packingBandage {count=10;name="ACE_packingBandage";};
     class _xx_ACE_elasticBandage {count=10;name="ACE_elasticBandage";};
    };
    class TransportWeapons{};
  };


  class ONI_Recon_AT_pack: ONI_Recon_Rucksack
  {
    scope=1;
    class TransportMagazines
    {
     class _xx_OPTRE_32Rnd_762x51_Mag {count=5;magazine="OPTRE_32Rnd_762x51_Mag";};
     class _xx_OPTRE_8Rnd_127x40_Mag {count=2;magazine="OPTRE_8Rnd_127x40_Mag";};
     class _xx_OPTRE_M41_Twin_HEAT_Thermal {count=3;magazine="OPTRE_M41_Twin_HEAT_Thermal";};
     class _xx_OPTRE_M41_Twin_HEAT_SALH {count=2;magazine="OPTRE_M41_Twin_HEAT_SALH";};
     class _xx_OPTRE_M41_Twin_HEAP {count=2;magazine="OPTRE_M41_Twin_HEAP";};
    };
    class TransportItems
    {
     class _xx_ACE_packingBandage {count=10;name="ACE_packingBandage";};
     class _xx_ACE_elasticBandage {count=10;name="ACE_elasticBandage";};
    };
    class TransportWeapons{};
  };


  class ONI_Recon_Medic_pack: ONI_Recon_Rucksack_Medic
  {
    scope=1;
    class TransportMagazines
    {
     class _xx_OPTRE_8Rnd_127x40_Mag {count=2;magazine="OPTRE_8Rnd_127x40_Mag";};
     class _xx_OPTRE_32Rnd_762x51_Mag {count=5;magazine="OPTRE_32Rnd_762x51_Mag";};
    };
    class TransportItems
    {
     class _xx_ACE_fieldDressing {count=40;name="ACE_fieldDressing";};
     class _xx_ACE_elasticBandage {count=40;name="ACE_elasticBandage";};
     class _xx_ACE_packingBandage {count=40;name="ACE_packingBandage";};
     class _xx_ACE_tourniquet {count=14;name="ACE_tourniquet";};
     class _xx_OPTRE_Biofoam {count=20;name="OPTRE_Biofoam";};
     class _xx_OPTRE_Medigel {count=15;name="OPTRE_Medigel";};
     class _xx_ACE_bloodIV {count=10;name="ACE_bloodIV";};
     class _xx_ACE_bloodIV_500 {count=20;name="ACE_bloodIV_500";};
     class _xx_ACE_surgicalKit {count=1;name="ACE_surgicalKit";};
     class _xx_ACE_bloodIV_250 {count=30;name="ACE_bloodIV_250";};
     class _xx_ACE_morphine {count=10;name="ACE_morphine";};
     class _xx_ACE_adenosine {count=10;name="ACE_adenosine";};
     class _xx_ACE_epinephrine {count=10;name="ACE_epinephrine";};
     class _xx_ACE_EarPlugs {count=5;name="ACE_EarPlugs";};
    };
    class TransportWeapons{};
  };


  class ONI_Recon_Team_Leader_pack: ONI_Recon_Rucksack_IC
  {
    scope=1;
    class TransportMagazines
    {
     class _xx_1Rnd_HE_Grenade_shell {count=5;magazine="1Rnd_HE_Grenade_shell";};
     class _xx_ACE_HuntIR_M203 {count=2;magazine="ACE_HuntIR_M203";};
     class _xx_OPTRE_M2_Smoke {count=5;magazine="OPTRE_M2_Smoke";};
     class _xx_OPTRE_32Rnd_762x51_Mag {count=4;magazine="OPTRE_32Rnd_762x51_Mag";};
     class _xx_OPTRE_8Rnd_127x40_Mag {count=1;magazine="OPTRE_8Rnd_127x40_Mag";};
     class _xx_ACE_CTS9 {count=1;magazine="ACE_CTS9";};
    };
    class TransportItems
    {
     class _xx_OPTRE_Medigel {count=5;name="OPTRE_Medigel";};
     class _xx_ACE_HuntIR_monitor {count=1;name="ACE_HuntIR_monitor";};
    };
    class TransportWeapons{};
  };


  class ONI_Recon_EOD_pack: ONI_Recon_Rucksack
  {
    scope=1;
    class TransportMagazines
    {
     class _xx_OPTRE_8Rnd_127x40_Mag {count=2;magazine="OPTRE_8Rnd_127x40_Mag";};
     class _xx_UNSCMine_Range_Mag {count=1;magazine="UNSCMine_Range_Mag";};
     class _xx_M168_Remote_Mag {count=1;magazine="M168_Remote_Mag";};
     class _xx_C12_Remote_Mag {count=1;magazine="C12_Remote_Mag";};
     class _xx_C7_Remote_Mag {count=1;magazine="C7_Remote_Mag";};
     class _xx_SatchelCharge_Remote_Mag {count=1;magazine="SatchelCharge_Remote_Mag";};
     class _xx_OPTRE_FC_BubbleShield {count=1;magazine="OPTRE_FC_BubbleShield";};
     class _xx_OPTRE_32Rnd_762x51_Mag {count=5;magazine="OPTRE_32Rnd_762x51_Mag";};
    };
    class TransportItems
    {
     class _xx_ACE_packingBandage {count=10;name="ACE_packingBandage";};
     class _xx_ACE_elasticBandage {count=10;name="ACE_elasticBandage";};
     class _xx_ToolKit {count=1;name="ToolKit";};
    };
    class TransportWeapons{};
  };


  class ONI_Recon_Marksman_pack: ONI_Recon_Rucksack
  {
    scope=1;
    class TransportMagazines{};
    class TransportItems
    {
     class _xx_ACE_packingBandage {count=10;name="ACE_packingBandage";};
     class _xx_ACE_elasticBandage {count=10;name="ACE_elasticBandage";};
    };
    class TransportWeapons{};
  };


  class ONI_Recon_LMG_pack: ONI_Recon_Rucksack
  {
    scope=1;
    class TransportMagazines
    {
     class _xx_OPTRE_8Rnd_127x40_Mag {count=2;magazine="OPTRE_8Rnd_127x40_Mag";};
     class _xx_OPTRE_200Rnd_95x40_Box {count=5;magazine="OPTRE_200Rnd_95x40_Box";};
    };
    class TransportItems
    {
     class _xx_ACE_packingBandage {count=10;name="ACE_packingBandage";};
     class _xx_ACE_elasticBandage {count=10;name="ACE_elasticBandage";};
    };
    class TransportWeapons{};
  };


  class ONI_Recon_MG_pack: ONI_Recon_Rucksack
  {
    scope=1;
    class TransportMagazines
    {
     class _xx_OPTRE_8Rnd_127x40_Mag {count=2;magazine="OPTRE_8Rnd_127x40_Mag";};
     class _xx_OPTRE_400Rnd_762x51_Box_Tracer {count=1;magazine="OPTRE_400Rnd_762x51_Box_Tracer";};
     class _xx_OPTRE_100Rnd_762x51_Box {count=4;magazine="OPTRE_100Rnd_762x51_Box";};
    };
    class TransportItems
    {
     class _xx_ACE_packingBandage {count=10;name="ACE_packingBandage";};
     class _xx_ACE_elasticBandage {count=10;name="ACE_elasticBandage";};
    };
    class TransportWeapons{};
  };


  class ONI_Recon_HMG_pack: ONI_Recon_Rucksack
  {
    scope=1;
    class TransportMagazines
    {
     class _xx_OPTRE_8Rnd_127x40_Mag {count=2;magazine="OPTRE_8Rnd_127x40_Mag";};
     class _xx_OPTRE_200Rnd_127x99_M247H_Etilka {count=4;magazine="OPTRE_200Rnd_127x99_M247H_Etilka";};
    };
    class TransportItems
    {
     class _xx_ACE_packingBandage {count=10;name="ACE_packingBandage";};
     class _xx_ACE_elasticBandage {count=10;name="ACE_elasticBandage";};
     class _xx_ACE_EarPlugs {count=1;name="ACE_EarPlugs";};
    };
    class TransportWeapons{};
  };


  class ONI_Recon_Rifleman_HMG_pack: ONI_Recon_Rucksack
  {
    scope=1;
    class TransportMagazines
    {
     class _xx_OPTRE_8Rnd_127x40_Mag {count=2;magazine="OPTRE_8Rnd_127x40_Mag";};
    };
    class TransportItems
    {
     class _xx_ACE_packingBandage {count=10;name="ACE_packingBandage";};
     class _xx_ACE_elasticBandage {count=10;name="ACE_elasticBandage";};
    };
    class TransportWeapons{};
  };


  class ONI_Recon_IC_HMG_pack: ONI_Recon_Rucksack_IC
  {
    scope=1;
    class TransportMagazines
    {
     class _xx_OPTRE_8Rnd_127x40_Mag {count=2;magazine="OPTRE_8Rnd_127x40_Mag";};
    };
    class TransportItems
    {
     class _xx_ACE_packingBandage {count=10;name="ACE_packingBandage";};
     class _xx_ACE_elasticBandage {count=10;name="ACE_elasticBandage";};
    };
    class TransportWeapons{};
  };


  class ONI_Recon_Medic_HMG_pack: ONI_Recon_Rucksack_Medic
  {
    scope=1;
    class TransportMagazines
    {
     class _xx_OPTRE_8Rnd_127x40_Mag {count=2;magazine="OPTRE_8Rnd_127x40_Mag";};
    };
    class TransportItems
    {
     class _xx_ACE_fieldDressing {count=40;name="ACE_fieldDressing";};
     class _xx_ACE_elasticBandage {count=40;name="ACE_elasticBandage";};
     class _xx_ACE_packingBandage {count=40;name="ACE_packingBandage";};
     class _xx_ACE_tourniquet {count=14;name="ACE_tourniquet";};
     class _xx_OPTRE_Biofoam {count=20;name="OPTRE_Biofoam";};
     class _xx_OPTRE_Medigel {count=15;name="OPTRE_Medigel";};
     class _xx_ACE_bloodIV {count=10;name="ACE_bloodIV";};
     class _xx_ACE_bloodIV_500 {count=20;name="ACE_bloodIV_500";};
     class _xx_ACE_surgicalKit {count=1;name="ACE_surgicalKit";};
     class _xx_ACE_bloodIV_250 {count=30;name="ACE_bloodIV_250";};
     class _xx_ACE_morphine {count=10;name="ACE_morphine";};
     class _xx_ACE_adenosine {count=10;name="ACE_adenosine";};
     class _xx_ACE_epinephrine {count=10;name="ACE_epinephrine";};
     class _xx_ACE_EarPlugs {count=5;name="ACE_EarPlugs";};
    };
    class TransportWeapons{};
  };


  class ONI_Recon_Sniper_pack: ONI_Recon_Rucksack_IC
  {
    scope=1;
    class TransportMagazines
    {
     class _xx_OPTRE_4Rnd_145x114_HEDP_Mag {count=6;magazine="OPTRE_4Rnd_145x114_HEDP_Mag";};
     class _xx_OPTRE_4Rnd_145x114_HVAP_Mag {count=5;magazine="OPTRE_4Rnd_145x114_HVAP_Mag";};
     class _xx_OPTRE_60Rnd_5x23mm_Mag {count=5;magazine="OPTRE_60Rnd_5x23mm_Mag";};
    };
    class TransportItems
    {
     class _xx_ACE_packingBandage {count=20;name="ACE_packingBandage";};
     class _xx_ACE_elasticBandage {count=20;name="ACE_elasticBandage";};
     class _xx_ACE_Tripod {count=1;name="ACE_Tripod";};
     class _xx_ACE_bloodIV_500 {count=5;name="ACE_bloodIV_500";};
     class _xx_ACE_bloodIV_250 {count=5;name="ACE_bloodIV_250";};
     class _xx_ACE_bloodIV {count=2;name="ACE_bloodIV";};
     class _xx_ACE_epinephrine {count=5;name="ACE_epinephrine";};
     class _xx_ACE_morphine {count=5;name="ACE_morphine";};
     class _xx_ACE_adenosine {count=5;name="ACE_adenosine";};
     class _xx_ACE_fieldDressing {count=10;name="ACE_fieldDressing";};
     class _xx_ACE_tourniquet {count=5;name="ACE_tourniquet";};
     class _xx_ACE_surgicalKit {count=1;name="ACE_surgicalKit";};
    };
    class TransportWeapons{};
  };


  class ONI_Recon_Spotter_pack: ONI_Recon_Rucksack_IC
  {
    scope=1;
    class TransportMagazines
    {
     class _xx_OPTRE_8Rnd_127x40_Mag {count=2;magazine="OPTRE_8Rnd_127x40_Mag";};
     class _xx_Commando_20Rnd_65_Mag {count=10;magazine="Commando_20Rnd_65_Mag";};
     class _xx_OPTRE_4Rnd_145x114_HEDP_Mag {count=5;magazine="OPTRE_4Rnd_145x114_HEDP_Mag";};
    };
    class TransportItems
    {
     class _xx_ACE_packingBandage {count=20;name="ACE_packingBandage";};
     class _xx_ACE_elasticBandage {count=20;name="ACE_elasticBandage";};
     class _xx_ACE_bloodIV_500 {count=5;name="ACE_bloodIV_500";};
     class _xx_ACE_bloodIV_250 {count=5;name="ACE_bloodIV_250";};
     class _xx_ACE_bloodIV {count=2;name="ACE_bloodIV";};
     class _xx_ACE_epinephrine {count=5;name="ACE_epinephrine";};
     class _xx_ACE_morphine {count=5;name="ACE_morphine";};
     class _xx_ACE_adenosine {count=5;name="ACE_adenosine";};
     class _xx_ACE_fieldDressing {count=10;name="ACE_fieldDressing";};
     class _xx_ACE_tourniquet {count=5;name="ACE_tourniquet";};
     class _xx_ACE_surgicalKit {count=1;name="ACE_surgicalKit";};
     class _xx_ACE_SpottingScope {count=1;name="ACE_SpottingScope";};
    };
    class TransportWeapons{};
  };